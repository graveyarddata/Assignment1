...
IMAGE   = os.getenv("PIPELINE_IMAGE")  # e.g., us-central1-docker.pkg.dev/..../penguins:sha

@component(base_image=IMAGE)
def preprocess_op(input_csv: str, out_dir: str):
    import subprocess
    subprocess.check_call(["python","components/preprocess_penguins.py","--input_csv",input_csv,"--out_dir",out_dir])

@component(base_image=IMAGE)
def train_op(train_csv: str, out_dir: str):
    import subprocess
    subprocess.check_call(["python","components/train_lr.py","--train_csv",train_csv,"--out_dir",out_dir])
...
def evaluate_and_promote_op(test_csv: str, model_pkl: str, out_dir: str, model_dir: str):
    ...
    subprocess.check_call([
        ...
        "--test_csv",test_csv,"--model_pkl",model_pkl,"--out_dir",out_dir,"--model_dir",model_dir
    ])

@dsl.pipeline(name="penguins-pipeline", pipeline_root=lambda: f"gs://{BUCKET}/runs")
...
if __name__ == "__main__":
    ...
    compiler.Compiler().compile(penguins_pipeline, "pipeline_spec.yaml", type_check=False)
...
PIPELINE_ROOT = f"gs://{BUCKET}/runs"            # <-- must be a plain string
